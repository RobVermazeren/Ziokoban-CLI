...
object ConsoleDrawing {
  ...
  private def toAnsiColor(c: BaseColor): AnsiColor =
    c match {
  ...
  object Draw {
    ...
    def setForeGroundColor(fg: Color): ConsoleDrawState[Unit] =
      State.modify { ansi =>
        if (fg.isBright)
    ...
    def setBackGroundColor(bg: Color): ConsoleDrawState[Unit] =
      State.modify { ansi =>
        if (bg.isBright)
    ...
    def drawChars(positions: Set[Coord], c: Char): ConsoleDrawState[Unit] =
      State.modify { ansi =>
        positions.foldLeft(ansi) {
          case (rAnsi, pos) =>
