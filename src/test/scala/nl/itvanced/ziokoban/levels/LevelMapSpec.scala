...
object LevelMapSpec extends DefaultRunnableSpec {
  ...
  object Generators {
    ...
    def coord(minX: Int = 0, maxX: Int = 40, minY: Int = 0, maxY: Int = 40): Gen[Random with Sized, Coord] =
      for {
        x <- Gen.int(minX, maxX)
        y <- Gen.int(minY, maxY)
      } yield Coord(x, y)

    def emptyFieldMap(maxWidth: Int, maxHeight: Int): Gen[Random with Sized, LevelFieldMap] =
      for {
        ...
        val fields = for {
          x <- 1 to width;
          y <- 1 to height
        } yield Coord(x, y) -> Field(Empty, false)
        fields.toMap
      }

    /** Generate field map, with one pusher */
    def fieldMap(maxWidth: Int, maxHeight: Int, maxCrates: Int): Gen[Random with Sized, LevelFieldMap] =
      for {
    ...
    def addOccupant(fm: LevelFieldMap, occupant: Occupant): Gen[Random with Sized, LevelFieldMap] =
      for {
    ...
    def addTarget(fm: LevelFieldMap): Gen[Random with Sized, LevelFieldMap] =
      for {
    ...
    def twoPusherFieldMap(maxWidth: Int, maxHeight: Int, maxCrates: Int = 0): Gen[Random with Sized, LevelFieldMap] =
      for {
        mapWithPusher <- fieldMap(maxWidth, maxHeight, maxCrates)
        if mapWithPusher.size > 2 // Ensure room enough for 2 pushers.
  ...
  def getCrateLocationsSpec =
    suite(".getCrateLocations")(
      ...
      testM("non-empty LevelMap") {
        check(Generators.fieldMap(maxWidth = 20, maxHeight = 20, maxCrates = 40)) {
          fieldMap =>
