...
class SLCTest extends FreeSpec with Matchers {
  ...
  object TestCases {
    ...
    def create(
      ...
      val descriptionLines: List[String] = description.map { d =>
        List(
          Some("<Description>"), 
    ...
    val invalidCases = List(
      ...
      "level without a line" -> create(levelCollection = () => levelCollection(levels = () => List(level(lines = () => Nil))))
    )
  }
  ...
  "SLC.loadFromString" - {
    ...
    TestCases.invalidCases.foreach { case (descr, testCase) =>
